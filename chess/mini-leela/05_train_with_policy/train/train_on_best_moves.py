"""
Training on Stockfish Top 5 Moves (with soft targets)
Configuration: 8 ResNet blocks, 256 channels
Target: Move probability distribution over top 5 moves

This combines:
- Good data: Positions with top 5 moves and their evaluations
- Good objective: Learn probability distribution over high-quality moves
- Soft targets: Convert evaluations to probabilities using softmax
"""
import chess
import torch
...
# DATA LOADING
TRAIN_CSV_FILE = "../data/train_enriched.csv"
VALIDATION_CSV_FILE = "../data/test_enriched.csv"

print("="*70)
print("TRAINING ON TOP 5 MOVES WITH SOFT TARGETS")
print("CONFIG: 8 ResNet blocks, 256 channels")
print("="*70)
print("\nTraining Targets:")
print("  - Policy: Probability distribution over top 5 moves")
print("  - Value: Position evaluation")
print("  - Loss: KL divergence for soft targets")
...
# Load training data with top 5 moves
print(f"Loading training data from {TRAIN_CSV_FILE}...")
training_positions = []
with open(TRAIN_CSV_FILE, 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        # Extract top 5 moves and their evaluations
        top_moves = []
        for i in range(1, 6):
            move_key = f'top_move_{i}'
            eval_key = f'top_eval_{i}'
            if move_key in row and eval_key in row and row[move_key]:
                top_moves.append((row[move_key], float(row[eval_key])))

        if top_moves:  # Only include if we have at least one move
            training_positions.append((row['fen'], row['answer'], float(row['stockfish_eval']), top_moves))

print(f"Loaded {len(training_positions):,} training positions")

# Load validation data with top 5 moves
...
with open(VALIDATION_CSV_FILE, 'r') as f:
    ...
    for row in reader:
        ...
        if top_moves:  # Only include if we have at least one move
            validation_positions.append((row['fen'], row['answer'], float(row['stockfish_eval']), top_moves))
...
def create_move_distribution(top_moves, temperature=1.0):
    """
    Convert move evaluations to probability distribution using softmax.

    Args:
        top_moves: List of (move_uci, eval) tuples
    ...
    # Extract evaluations
    evals = np.array([eval_score for _, eval_score in top_moves])

    # Apply softmax with temperature
    # Higher evals should get higher probability
    exp_evals = np.exp(evals / temperature)
    probs = exp_evals / np.sum(exp_evals)

    return [(move, prob) for (move, _), prob in zip(top_moves, probs)]


# Prepare training data with soft targets
print("Preparing training data...")
train_data = []
TEMPERATURE = 2.0  # Temperature for softmax (higher = softer distribution)

for fen, solution_uci, stockfish_eval, top_moves in training_positions:
    board = chess.Board(fen)
    board_state = encoder.encode_board(board)

    # Create soft targets for source and destination squares
    source_target = np.zeros(64, dtype=np.float32)
    dest_target = np.zeros(64, dtype=np.float32)

    # Convert top moves to probability distribution
    move_probs = create_move_distribution(top_moves, temperature=TEMPERATURE)

    # Distribute probabilities across source and destination squares
    for move_uci, prob in move_probs:
        try:
            move = chess.Move.from_uci(move_uci)
            # Add probability to source and dest squares
            source_target[move.from_square] += prob
            dest_target[move.to_square] += prob
        except:
            # Skip invalid moves
            continue

    # Normalize to ensure they sum to 1 (in case of overlapping sources/dests)
    if source_target.sum() > 0:
        source_target /= source_target.sum()
    if dest_target.sum() > 0:
        dest_target /= dest_target.sum()
...
def train_epoch(data, network, optimizer, batch_size, device):
    ...
    for i in range(0, len(data), batch_size):
        ...
        # For numerical stability, use: sum(P * log(P)) - sum(P * log(Q))
        source_log_probs = F.log_softmax(source_logits, dim=1)
        dest_log_probs = F.log_softmax(dest_logits, dim=1)

        source_loss = -(source_targets * source_log_probs).sum(dim=1).mean()
        dest_loss = -(dest_targets * dest_log_probs).sum(dim=1).mean()
...
def evaluate(positions, network, device):
    ...
    with torch.no_grad():
        for fen, solution_uci, stockfish_eval, top_moves in positions:
            board = chess.Board(fen)
            ...
            # Create soft targets for evaluation (same as training)
            source_target = np.zeros(64, dtype=np.float32)
            dest_target = np.zeros(64, dtype=np.float32)
            ...
            source_target_tensor = torch.FloatTensor(source_target).unsqueeze(0).to(device)
            dest_target_tensor = torch.FloatTensor(dest_target).unsqueeze(0).to(device)

            source_log_probs = F.log_softmax(source_logits, dim=1)
            dest_log_probs = F.log_softmax(dest_logits, dim=1)

            source_loss = -(source_target_tensor * source_log_probs).sum(dim=1).mean()
            dest_loss = -(dest_target_tensor * dest_log_probs).sum(dim=1).mean()
            ...
            # For accuracy, still check if best move matches the top move
...
print("\n" + "="*70)
print("TRAINING COMPLETE - Top 5 Moves with Soft Targets")
print("="*70)
print(f"Temperature: {TEMPERATURE}")
